...
<input type="button" class="ui-button ui-widget ui-state-default ui-corner-all" value="<spring:message code='inventory.indent.add'/>" onclick="ACT.go('subStoreIndentDrug.form');"/>
...
<c:forEach items="${listIndent}" var="indent" varStatus="varStatus">
	<tr class='${varStatus.index % 2 == 0 ? "oddRow" : "evenRow" } '>
		...
		<c:if test="${indent.subStoreStatus == 3 }">
			<a href="#" onclick="ACT.go('subStoreDrugProcessIndent.form?indentId=${ indent.id}');"><spring:message code="inventory.indent.process"/></a>
		</c:if>
		<label></label>
		</td>
	</tr>
